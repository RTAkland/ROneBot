...
fun <T : IBaseCommand<IGroupMessage, IPrivateMessage>> T.hasPermission(userId: String, permNode: String): Boolean {
    return permissionManager.hasPermission(userId, permNode)
}

fun <T : IBaseCommand<IGroupMessage, IPrivateMessage>> T.hasPermission(userId: String, level: Int): Boolean {
 ...
 * 在命令上下文中设置用户的权限
 */
fun <T : IBaseCommand<IGroupMessage, IPrivateMessage>> T.setPermission(userId: String, permission: BasicPermission) {
    permissionManager.setUserPermission(userId, permission)
}

fun <T : IBaseCommand<IGroupMessage, IPrivateMessage>> T.setPermission(userId: String, permNode: String) {
    permissionManager.setUserPermission(userId, permNode)
}

fun <T : IBaseCommand<IGroupMessage, IPrivateMessage>> T.setPermission(userId: String, level: Int) {
 ...
 * 撤销一个用户的[BasicPermission]和权限等级
 */
fun <T : IBaseCommand<IGroupMessage, IPrivateMessage>> T.revokePermission(userId: String) {
 ...
 * 撤销一个用户的权限节点
 */
fun <T : IBaseCommand<IGroupMessage, IPrivateMessage>> T.revokePermission(userId: String, permNode: String) {
